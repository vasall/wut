...
WT_API void wt_element_render(struct wt_batch *ren, struct wt_element *ele)
{
	 ...
	 */
	if(!(ele->info_flags & WT_ELEMENT_F_VISIBLE))
		return;
	...
	/* Unioform: u_rect */
	v_index[0] = wt_batch_push_uniform(ren, 1, &ele->element_rect);

	/* Uniform: u_color */
	wt_color_get_fv(ele->style.infill_color, color);
	v_index[2] = wt_batch_push_uniform(ren, 2, color);

	/* Uniform: u_radius */
	wt_batch_push_uniform(ren, 3, ele->style.radius_corner);

	/* Uniform: u_bwidth */
	wt_batch_push_uniform(ren, 4, &ele->style.border_width);

	/* Uniform: u_bcolor */
	wt_color_get_fv(ele->style.border_color, color);
	wt_batch_push_uniform(ren, 5, color);

	/* Uniform: u_limit */
	if(ele->parent) {
		v_index[1] = wt_batch_push_uniform(ren, 7, &ele->parent->inner_rect);
	}
	else {
		v_index[1] = -1;
	}
...
WT_XMOD void wt_element_ren_scrollbar(struct wt_batch *ren, struct wt_element *ele)
{
	...
	/* vertical */
	if(ele->scrollbar_flags & WT_RESTYLE_SCROLL_V) {
		...
		/* Unioform: u_rect */
		s_index[0] = wt_batch_push_uniform(ren, 1, &rect);
		...
		s_index[2] = wt_batch_push_uniform(ren, 6, scroll);

		vdata.z = (f32)ele->layer / 100.0;
		vdata.index[0] = s_index[0];
		vdata.index[1] = -1;
		vdata.index[2] = s_index[2];
		vdata.type = WT_RENTYPE_SCROLL_V;
	...
	/* horizontal */
	if(ele->scrollbar_flags & WT_RESTYLE_SCROLL_H) {
		...
		/* Uniform: u_scroll */
		s_index[2] = wt_batch_push_uniform(ren, 6, h_scroll);

		vdata.z = (f32)ele->layer / 100.0;
		vdata.index[0] = s_index[0];
		vdata.index[1] = -1;
		vdata.index[2] = s_index[2];
		vdata.type = WT_RENTYPE_SCROLL_H;
